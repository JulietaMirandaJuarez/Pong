...
public class MenuScreen implements Screen {
    ...
    private String[] options = {"Play online", "Exit"};
    ...
    private void udpate() {
        ...
        if(this.keyListener.isEnterJustPressed()){
            if(this.options[this.selected].equals("Play online")) {
    ...
    @Override
    public void dispose() {
        this.title.getFont().dispose();
        this.copyright.getFont().dispose();
        for (Text option : this.menuOptions) {
            option.getFont().dispose();
        }
    }
}
